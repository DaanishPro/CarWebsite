...
import { X, Calendar, MapPin, DollarSign, Car, Mail, Phone, User2, CheckCircle, CarFront } from "lucide-react"
...
<<<<<<< HEAD
    ...
    if (!formData.fullName || !formData.phoneNumber || !formData.emailAddress || !formData.preferredVariant || !formData.bookingDate || !formData.city || !formData.paymentPreference || !formData.agreedToTerms) {
    ...
    const success = await databaseUtils.saveBookingData(user.uid, bookingData)
    setIsSubmitting(false)
        ...
        >
          <Card className="rounded-3xl border-none">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <div>
                <CardTitle className="text-2xl font-bold font-display text-theme-accent-900">Book Your {carData.name}</CardTitle>
