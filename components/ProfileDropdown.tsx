...
export default function ProfileDropdown({ isMobile = false }: ProfileDropdownProps) {
  ...
  // ✅ Fetch booked vehicles (Fixed price logic)
  useEffect(() => {
        ...
        (snapshot) => {
          if (snapshot.exists()) {
            ...
            const formattedBookings: BookedCar[] = Object.entries(bookingsData)
              .map(([id, data]: any) => {
                // ✅ Price fix: read from multiple possible keys
                const price =
                  data.price ??
                  data.amount ??
                  data.totalPrice ??
                ...
                // ✅ Discount optional fix
                const discount = data.discount ?? data.offer ?? 0
  ...
  }, [user, isVehiclesModalOpen, profileData.fullName])
            ...
            {isLoadingVehicles ? (
              ...
              <div className="space-y-4">
                {bookedCars.map((car) => (
                  <Card
                    key={car.bookingId}
                    className="border-l-4 border-red-500 shadow-sm hover:shadow-lg transition"
                  >
